...
//   name: string;
//   deadline: Date;
//   expectedCompletionTime: number; // in hours
//   completionLevel: number; // between 0 to 1
//   priority: number; // between 0 to 1
// }
...
//     repeatTime: string
//   ) {
//     assert(this.events.includes(oldEvent));
//     oldEvent.name = name;
//     oldEvent.startTime = startTime;
//     oldEvent.endTime = endTime;
//     oldEvent.repeatTime = repeatTime;
//   }

//   deleteEvent(event: Event) {
//     assert(this.events.includes(event));
//     this.events = this.events.filter((e) => e != event); // Removes specified event
//   }
...
//     priority: number
//   ) {
//     assert(this.tasks.includes(oldTask));
//     oldTask.name = name;
//     oldTask.deadline = deadline;
//     oldTask.expectedCompletionTime = expectedCompletionTime;
//     oldTask.completionLevel = completionLevel;
//     oldTask.priority = priority;
//   }

//   deleteTask(task: Task) {
//     assert(this.tasks.includes(task));
//     this.tasks = this.tasks.filter((t) => t != task); // Removes specified task
//   }

//   async generateSchedule(llm: GeminiLLM): Promise<void> {
//     try {
//       console.log("🤖 Requesting new schedule from Gemini AI...");

//       const unassignedActivities = this.activities.filter(
//         (a) => !this.isAssigned(a)
//       );
...
//       "3. Avoid conflicts - don't overlap events and/or tasks",
//       "4. Consider the expectedCompletionTime of each activity when scheduling",
...
//         `${
//           criticalRequirements.length + 1
//         }. Keep the existing assignments listed above exactly as they are (no overlaps or changes)`
//       );
//     }

//     return `
// You are a helpful AI assistant that creates optimal daily schedules for students given events and tasks.

// STUDENT PREFERENCES:
// - Study time, homework, and project work should ideally be scheduled during focused hours (9:00 AM - 5:00 PM)
// - Meals should ideally be at regular intervals (breakfast 7-9 AM, lunch 12-1 PM, dinner 6-8 PM)
// - Social activities and relaxation are good for evenings (6:00 PM - 10:00 PM)
// - Try to avoid scheduling demanding activities too late at night (after 10:00 PM)
// - Try to leave buffer time between different types of activities
...
//         for (
//           let offset = 0;
//           offset < existingAssignment.activity.duration;
//           offset++
//         ) {
//           occupiedSlots.set(
//             existingAssignment.startTime + offset,
//             existingAssignment.activity
//           );
//         }
//       }
...
//         const { title, startTime } = rawAssignment as {
//           title?: unknown;
//           startTime?: unknown;
//         };
...
//           issues.push(
//             `No available occurrences of activity "${title}" to assign.`
//           );
//           continue;
//         }
...
//           issues.push(
//             `Activity "${title}" has an out-of-range start time (${startTime}).`
//           );
//           continue;
//         }
...
//         if (endSlot > 48) {
//           issues.push(
//             `Activity "${title}" would extend past the end of the day.`
//           );
//           continue;
//         }
...
//             issues.push(
//               `Time slot ${this.formatTimeSlot(slot)} is already taken by "${
//                 occupyingActivity.title
//               }" and conflicts with "${title}".`
//             );
...
//       if (issues.length > 0) {
//         throw new Error(
//           `LLM provided disallowed assignments:\n- ${issues.join("\n- ")}`
//         );
//       }
...
//         console.log(
//           `✅ Assigned "${assignment.activity.title}" to ${this.formatTimeSlot(
...
//         const durationStr =
//           assignment.activity.duration === 1
//             ? "30 minutes"
//             : `${assignment.activity.duration * 0.5} hours`;
...
//         const isActivityStart = activities.some(
//           (activity) =>
//             this.assignments.find((a) => a.activity === activity)?.startTime ===
//             slot
//         );
